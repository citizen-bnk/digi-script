...
read it before starting any frontend work. It flags an unresolved conflict
(the deck mixes two different visual/branding systems) and two roles
(Scanning Team, Student login) not covered by the specs above.
...
- `assertCanAccessStudent` (`src/modules/students/student.service.ts`) is
  ...
  `TEACHER`/`SUPERVISOR` are scoped to `assignedClassName`, and `PARENT`
  requires a `ParentStudentLink`. **Known gap:** the document module does
...
- The Scanning Team (data-capturing) role and student-login portal shown
  in the screenshot deck — neither appears in the PRD's role list
